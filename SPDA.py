...
if df is not None:
    ...
    fig_pie_seller_type = px.pie(values=seller_type_distribution.values, names=seller_type_distribution.index, title="Distribution by Seller Type")
    ...
    fig_pie_fuel_type = px.pie(values=fuel_distribution.values, names=fuel_distribution.index, title="Distribution by Fuel Type")
    st.plotly_chart(fig_pie_fuel_type, use_container_width=True)

    st.markdown("### Box Plot: Selling Price by Fuel Type")
    fig_box_fuel_price = px.box(df_selection, x="fuel", y="selling_price", color="fuel", title="Selling Price Distribution by Fuel Type")
    st.plotly_chart(fig_box_fuel_price, use_container_width=True)

    st.markdown("### Box Plot: Selling Price by Car Brand")
    fig_box_brand_price = px.box(df_selection, x="name", y="selling_price", color="name", title="Selling Price Distribution by Car Brand")
    st.plotly_chart(fig_box_brand_price, use_container_width=True)

    st.markdown("### Scatter Plot: Engine Size vs. Selling Price")
    fig_engine_vs_price = px.scatter(df_selection, x="Engine (CC)", y="selling_price", color="fuel", title="Engine Size vs. Selling Price")
